...
def addProduct():
    if request.method=='POST':
        ...
        con=mysql.connection.cursor()
        sql="insert into products (Product,Warehouse,Quantity) value (%s,%s,%s)"
        con.execute(sql,[product,quantity,warehouse])
...
def editDetails(id):
    ...
    if request.method=='POST':
        ...
        warehouse=request.form['warehouse']
        quantity=request.form['quantity']
        sql="update products set Product=%s,Warehouse=%s Quantity=%s where Id=%s"
        con.execute(sql,[product,warehouse,quantity,id])
        mysql.connection.commit()
        con.close()
